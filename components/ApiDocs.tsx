...
mediator within ~30 seconds. The mediator will automatically 
push the subaddress to the payment record. Poll GET /api/payments/:id 
to check payment status - the "address" field will be included 
once the subaddress is generated and pushed by the mediator.`}
                </pre>
              </div>
...
- "failed": Payment failed`}
              ...
              <div>
                ...
                <pre className="bg-gray-900/50 border border-gray-800 p-4 rounded-lg text-sm overflow-x-auto text-gray-300 font-mono backdrop-blur-sm">
{`Response (200 OK) - when subaddress is available:
{
  "paymentId": "uuid",
  "subaddress": "8xxxxx..."
}

Response (404 Not Found) - when subaddress not yet generated:
{
...
Note: The mediator generates subaddresses asynchronously and pushes 
them to the payment record. This endpoint returns the subaddress 
once it's been pushed by the mediator (usually within ~30 seconds). 
Alternatively, use GET /api/payments/:id which includes the 
subaddress in the "address" field.`}
                </pre>
              </div>
            </div>
          </section>
