import { NextRequest, NextResponse } from 'next/server';
import dbConnect from '@/lib/db';
import { requireInternalSecret } from '@/lib/auth';
import User from '@/lib/models/User';
import { assignAccountSchema } from '@/types';

export const dynamic = 'force-dynamic';

/**
 * POST /api/internal/assign-account
 * Assign an account index to a user
 */
export async function POST(request: NextRequest) {
  try {
    const authCheck = requireInternalSecret(request);
    if (authCheck) {
      return authCheck;
    }

    await dbConnect();

    const body = await request.json();
    const validationResult = assignAccountSchema.safeParse(body);

    if (!validationResult.success) {
      return NextResponse.json(
        {
          error: 'Validation error',
          code: 'VALIDATION_ERROR',
          details: validationResult.error.errors,
        },
        { status: 400 }
      );
    }

    const { publicKey, accountIndex } = validationResult.data;

    const user = await User.findOne({ publicKey });

    if (!user) {
      return NextResponse.json(
        { error: 'User not found', code: 'NOT_FOUND' },
        { status: 404 }
      );
    }

    if (user.accountIndex !== undefined && user.accountIndex !== null) {
      return NextResponse.json(
        {
          error: 'User already has an account assigned',
          code: 'ALREADY_ASSIGNED',
        },
        { status: 400 }
      );
    }

    // Check if account index is already assigned to another user
    const existingUser = await User.findOne({ accountIndex });
    if (existingUser) {
      return NextResponse.json(
        {
          error: 'Account index already assigned to another user',
          code: 'DUPLICATE_ACCOUNT_INDEX',
        },
        { status: 409 }
      );
    }

    // Assign account index
    user.accountIndex = accountIndex;
    await user.save();

    return NextResponse.json({
      success: true,
      user: {
        publicKey: user.publicKey,
        accountIndex: user.accountIndex,
      },
    });
  } catch (error: any) {
    console.error('Error assigning account:', error);

    if (error.code === 11000) {
      return NextResponse.json(
        {
          error: 'Account index already assigned',
          code: 'DUPLICATE_ACCOUNT_INDEX',
        },
        { status: 409 }
      );
    }

    return NextResponse.json(
      { error: 'Internal server error', code: 'INTERNAL_ERROR' },
      { status: 500 }
    );
  }
}

