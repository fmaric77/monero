...
export async function POST(request: NextRequest) {
  try {
    ...
    // Subaddress will be generated asynchronously by the mediator
    // The mediator polls for new payments every ~30 seconds and assigns subaddresses automatically
    console.log(`ℹ️ Payment created, subaddress will be assigned by mediator within ~30 seconds`);

    // Payment response - subaddress will be null initially, generated by mediator asynchronously
    const paymentResponse: PaymentResponse = {
      id: payment.paymentId,
      amount: payment.amount,
      status: payment.status,
      expiresAt: payment.expiresAt.toISOString(),
      // address will be null - use GET /api/payments/:id to fetch once generated
    };
